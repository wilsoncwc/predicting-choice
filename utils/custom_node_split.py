...
class CustomNodeSplit(BaseTransform):
    ...
    def _split(self, store: NodeStorage) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        if self.mask_label:
            mask = store[self.mask_label]
            ids = torch.nonzero(mask)
            num_split_nodes = int(ids.size(0))
        else:
            num_split_nodes = store.num_nodes
        
        print(num_split_nodes)
        ...
        if isinstance(self.num_val, float):
            num_val = round(num_split_nodes * self.num_val)
        else:
            num_val = self.num_val

        if isinstance(self.num_test, float):
            num_test = round(num_split_nodes * self.num_test)
        else:
            num_test = self.num_test

        if self.split == 'train_rest':
            split_perm = torch.randperm(num_split_nodes)
            perm = torch.index_select(ids, 0, split_perm)
            if not self.mask_label:
                assert split_perm.equal(perm)
